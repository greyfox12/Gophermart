...
func LoginPage(db *sql.DB, authGen hash.AuthGen) http.HandlerFunc {
	return func(res http.ResponseWriter, req *http.Request) {
		...
		if err != nil && n <= 0 {
			...
			logmy.OutLog(fmt.Errorf("logingpage: read body request: %w", err))
		...
		if err != nil {
			...
			logmy.OutLog(fmt.Errorf("logingpage: decode json: %w", err))
			res.WriteHeader(400)
			return
		}
		if vRegister.Login == "" || vRegister.Password == "" {
			fmt.Printf("Error login/passwd %v/%v \n", vRegister.Login, vRegister.Password)
			logmy.OutLog(fmt.Errorf("logingpage: login/passwd: %v/%v", vRegister.Login, vRegister.Password))
			res.WriteHeader(400)
			return
		}
		...
		ret, err := dbstore.Loging(ctx, db, vRegister.Login, vRegister.PasswordMD5)
		if err != nil {
			fmt.Printf("Error logingpage %v \n", err)
			logmy.OutLog(fmt.Errorf("logingpage: db register: %w", err))
			res.WriteHeader(http.StatusBadRequest)
			return
		}
		...
		if ret == 200 {
			...
			if err != nil {
				logmy.OutLog(fmt.Errorf("logingpage: create token: %w", err))
				res.WriteHeader(http.StatusBadRequest)
				return
			}
